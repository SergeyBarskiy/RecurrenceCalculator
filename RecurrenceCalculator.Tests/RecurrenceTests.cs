...
namespace RecurrenceCalculator.Tests
{
    ...
    public class RecurrenceTests
    {
        ...
        [TestMethod]
        public void Should_Compute_Monthly_Recurrence_Start_Date_Greater_Then_Day_Scheduled()
        {
            var recurrence = new AppointmentRecurrence
            {
            ...
            var specificCalendar = new Calculator(DayOfWeek.Monday);
            var occurrences = specificCalendar.CalculateOccurrences(recurrence).ToList();
            Assert.AreEqual(2, occurrences.Count, "Should create 1 occurrence");
            Assert.AreEqual(new DateTime(2023, 02, 18), occurrences[0], "Date 1 should be correct");
            Assert.AreEqual(new DateTime(2023, 03, 18), occurrences[1], "Date 3 should be correct");
        }

        [TestMethod]
        public void Should_Compute_Monthly_Recurrence_Start_Date_Less_Then_Day_Scheduled()
        {
            var recurrence = new AppointmentRecurrence
            {
                RecurrenceType = RecurrenceType.Monthly,
